...
# Copyright (C) 2019 Ottavio Miele (ottavio97)
...
while true; do
	...
	if [ "$choice" == "1" ]; then
		...
		git clone https://android.googlesource.com/platform/prebuilts/gcc/linux-x86/aarch64/aarch64-linux-android-4.9 -b android-8.1.0_r65 --depth=1 stock 
	...
	if [ "$choice" == "3" ]; then
		echo -e "\n#######################################################################"
        echo -e "(i) Make-cleaning..."
		make O=out clean
        echo -e "(i) Mrproper-ing..."
		make mrproper
        echo -e "(i) Deleting folder..."
		rm -rf out/*
